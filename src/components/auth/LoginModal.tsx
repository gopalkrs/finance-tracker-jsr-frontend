...
import type { LoginInputs, SignupInputs } from "../../types/input";
...
const LoginModal = ({ setShowLoginModal, authMode, setAuthMode }: any) => {
  ...
  const onSubmitHandler : SubmitHandler<LoginInputs | SignupInputs> = async (data) => {
    console.log(data);
    const responseData = await axios.post('http://localhost:8080/api/users/create', data);
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<SignupInputs | LoginInputs>();
